...
int main(void) {
	...
	if ((uint16_t)result != 0xCA1C) {
		EEPROM_WriteVariable(EEPROM_VAR_BCKL, (EEPROM_Value){.uInt16 = ((DEFAULT_LCD_BL << 8) | DEFAULT_KBD_BL)}, EEPROM_SIZE16);
		EEPROM_WriteVariable(EEPROM_VAR_KBD, (EEPROM_Value){.uInt32 = ((DEFAULT_KBD_DEB << 16) | DEFAULT_KBD_FREQ)}, EEPROM_SIZE32);
		EEPROM_WriteVariable(EEPROM_VAR_CFG, (EEPROM_Value){.uInt16 = (((CFG_USE_MODS | CFG_REPORT_MODS | CFG_RST_DELAY_1S) << 8) | (INT_OVERFLOW | INT_KEY | INT_RTC | INT_PWR_BTN))}, EEPROM_SIZE16);
		EEPROM_WriteVariable(EEPROM_VAR_ID, (EEPROM_Value){.uInt16 = 0xCA1C}, EEPROM_SIZE16);
#ifdef DEBUG
