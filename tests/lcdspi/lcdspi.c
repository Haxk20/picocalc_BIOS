...
void lcd_printf_string(char* s, ...) {
	...
	buffer[255] = 0;	// To avoid undefined behavior
...
uint32_t lcd_spi_init() {
    ...
    // init SPI
    speed = spi_init(Pico_LCD_SPI_MOD, LCD_SPI_SPEED);
...
uint32_t lcd_init() {
    uint32_t spi_speed = lcd_spi_init();
    pico_lcd_init();
